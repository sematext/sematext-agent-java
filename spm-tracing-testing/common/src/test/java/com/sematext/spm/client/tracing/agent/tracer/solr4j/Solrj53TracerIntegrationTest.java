...
public class Solrj53TracerIntegrationTest {
    ...
    for (Object names : response.getResponse().getAll("collections")) {
      for (String name : (List<String>) names) {
        if (name.equals("test-collection-1") || name.equals("test-collection-2")) {
          new CollectionAdminRequest.Delete().setCollectionName(name).process(client);
        }
      }
    }

    new CollectionAdminRequest.Create().setCollectionName("test-collection-1").setNumShards(1).process(client);
    new CollectionAdminRequest.Create().setCollectionName("test-collection-2").setNumShards(1).process(client);
  }
  ...
  public void testRequestTypes() throws Exception {
    ...
    testRequestType(fieldsRequest, "test-collection-1", SolrAnnotation.RequestType.SCHEMA);
