...
public class Solrj52TracerIntegrationTest {
  ...
  public void preStart() throws Exception {
    ...
    for (Object names : response.getResponse().getAll("collections")) {
      for (String name : (List<String>) names) {
        if (name.equals("test_collection_1") || name.equals("test_collection_2")) {
    ...
    create1Request.setCollectionName("test_collection_1");
    ...
    create2Request.setCollectionName("test_collection_2");
  ...
  public void testRequestTypes() throws Exception {
    SolrAnnotation annotation = testRequestType(new QueryRequest(new MapSolrParams(singletonMap("q", "*.*"))), "test_collection_1", SolrAnnotation.RequestType.QUERY);
    final SolrInputDocument document = new SolrInputDocument();
    document.addField("firstName", "Corey");
    document.addField("lastName", "Taylor");
    annotation = testRequestType(new UpdateRequest()
                                     .add(document), "test_collection_1", SolrAnnotation.RequestType.UPDATE);
    assertEquals("test_collection_1", annotation.getCollection());
    ...
    annotation = testRequestType(contentStreamRequest, "test_collection_1", SolrAnnotation.RequestType.UPDATE);
    assertEquals("test_collection_1", annotation.getCollection());

    final DirectXmlRequest xmlRequest = new DirectXmlRequest("/update", XML_DOC);
    annotation = testRequestType(xmlRequest, "test_collection_1", SolrAnnotation.RequestType.UPDATE);
    assertEquals("test_collection_1", annotation.getCollection());
  ...
  public void testConcurrentUpdate() throws Exception {
    ...
    final ConcurrentUpdateSolrClient client = new ConcurrentUpdateSolrClient("http://localhost:8983/solr/test_collection_1", 2, 10) {
      @Override
    ...
    for (int i = 0; i < 10; i++) {
      ...
      new UpdateRequest("/test_collection_1/update").add(document).process(client, "test_collection_1");
    }
