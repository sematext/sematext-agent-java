...
@TracingContext(tracers = { Tracers.Solrj4Tracer.class, Tracers.HttpClient4Tracer.class })
public class Solr4103TracerIntegrationTest {

  private SolrServer getSolrClient() {
    return getSolrClient(null);
  }

  private SolrServer getSolrClient(String collection) {
  ...
  public void preStart() throws Exception {
    final SolrServer client = getSolrClient();
    CollectionAdminRequest listAction = new CollectionAdminRequest();
    listAction.setAction(CollectionParams.CollectionAction.LIST);
    CollectionAdminResponse response = listAction.process(client);
  ...
  public void testRequestTypes() throws Exception {
    testRequestType(new QueryRequest(new MapSolrParams(singletonMap("q", "*.*"))), "test-collection-1", SolrAnnotation.RequestType.QUERY);

    final SolrInputDocument document = new SolrInputDocument();
    document.addField("firstName", "Corey");
    document.addField("lastName", "Taylor");
    SolrAnnotation annotation = testRequestType(new UpdateRequest()
    ...
    contentStreamRequest.addContentStream(new ContentStreamBase.StringStream(XML_DOC));
    ...
    CollectionAdminRequest listAction = new CollectionAdminRequest();
    listAction.setAction(CollectionParams.CollectionAction.LIST);
    testRequestType(listAction, null, SolrAnnotation.RequestType.COLLECTION_ADMIN);
  }
  ...
  public void testConcurrentUpdate() throws Exception {
    ...
    final ConcurrentUpdateSolrServer client = new ConcurrentUpdateSolrServer("http://localhost:8983/solr/test-collection-1", 2, 10) {
      @Override
    ...
    for (int i = 0; i < 10; i++) {
      ...
      new UpdateRequest("/test-collection-1/update").add(document).process(client);
    }
