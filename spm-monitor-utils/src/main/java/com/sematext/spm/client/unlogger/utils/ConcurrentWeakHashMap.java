 ...
 * reclamation. If weak values are desired, one can simply use a {@link java.lang.ref.WeakReference} for the value type.
 * <p>
 ...
 * iterator/enumeration. They do <em>not</em> throw {@link java.util.ConcurrentModificationException}. However, iterators are
 ...
 * <p>
 * <p>
 * This class and its views and iterators implement all of the <em>optional</em> methods of the {@link java.util.Map} and
 * {@link java.util.Iterator} interfaces.
 * <p>
 * <p>
 * Like {@link java.util.Hashtable} but unlike {@link java.util.HashMap}, this class does <em>not</em> allow <tt>null</tt> to be used as a
 * key or value.
 * <p>
 * <p>
 ...
 */
public class ConcurrentWeakHashMap<K, V> extends AbstractMap<K, V> implements java.util.concurrent.ConcurrentMap<K, V>,
    Serializable {
   ...
   * is critical because ConcurrentWeakHashMap uses power-of-two length hash tables, that otherwise encounter collisions
   ...
   * to see null instead of initial value when read via a data race. Although a reordering leading to this is not likely
   * to ever actually occur, the Segment.readValueUnderLock method is used as a backup in case a null (pre-initialized)
  ...
  static final class Segment<K, V> extends ReentrantLock implements Serializable {
     ...
     * write operations performed by other threads are noticed. For most purposes, the "count" field, tracking the
     * number of elements, serves as that volatile variable ensuring visibility. This is convenient because this field
     * needs to be read in many read operations anyway:
     *
     * - All (unsynchronized) read operations must first read the "count" field, and should not look at table entries if
     * it is 0.
     *
     * - All (synchronized) write operations should write to the "count" field after structurally changing any bin. The
     * operations must not take any action that could even momentarily cause a concurrent read operation to see
   ...
   * @param loadFactor       the load factor threshold, used to control resizing. Resizing may be performed when the average number of
   *                         elements per bin exceeds this threshold.
   * @param concurrencyLevel the estimated number of concurrently updating threads. The implementation performs internal sizing to try
