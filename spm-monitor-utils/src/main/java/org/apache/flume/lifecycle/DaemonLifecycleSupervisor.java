...
/**
 * Our custom variant of Flume's LifecycleSupervisor which creates daemon supervisor threads (instead of non-daemon
 * which original does)
 */
public class DaemonLifecycleSupervisor implements LifecycleAware {

  private static final Logger logger = LoggerFactory
      .getLogger(DaemonLifecycleSupervisor.class);
  ...
  public DaemonLifecycleSupervisor() {
    ...
    monitorService = new ScheduledThreadPoolExecutor(10,
                                                     new ThreadFactoryBuilder().setNameFormat(
                                                         "lifecycleSupervisor-" + Thread.currentThread().getId()
                                                             + "-%d").setDaemon(true)
                                                         .build());
