...
import org.apache.flume.channel.file.DaemonFileChannel;
...
/**
 * Our custom version of SpillableMemoryChannel which will (indirectly) create daemon threads instead of non-deamon which is
 * done by Flume (this is then causing issues with JVM shutdown in javaagent setups).
 */
@InterfaceAudience.Private
@InterfaceStability.Evolving
@Recyclable
public class DaemonSpillableMemoryChannel extends DaemonFileChannel {
  private static final Logger LOG = LoggerFactory
      .getLogger(DaemonSpillableMemoryChannel.class);
  // config settings
  /**
  ...
  private static Logger LOGGER = LoggerFactory.getLogger(DaemonSpillableMemoryChannel.class);
  ...
  public DaemonSpillableMemoryChannel() {
    super();
  }
    ...
    protected void doCommit() throws InterruptedException {
      if (putCalled) {
        ...
        if (LOGGER.isDebugEnabled()) {
          // adjusted because this was causing ConcurrentModificationException in some cases and it is 
