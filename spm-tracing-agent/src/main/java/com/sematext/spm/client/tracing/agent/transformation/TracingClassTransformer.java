...
public class TracingClassTransformer implements ClassFileTransformer {
  ...
  private static final Log LOG = LogFactory.getLog(TracingClassTransformer.class);

  private final List<TracingTransform> transforms;

  public TracingClassTransformer(List<TracingTransform> transforms) {
    this.transforms = transforms;
  }

  @Override
  public byte[] transform(ClassLoader loader, String className, Class<?> klass,
                          ProtectionDomain prot, byte[] buff) throws IllegalClassFormatException {

    try {
      final CtClass ctClass = new ClassPools().getClassPool(loader).makeClass(new ByteArrayInputStream(buff));
      ...
      if (LOG.isTraceEnabled()) {
        LOG.trace("Error while retransforming " + className + ".", e);
      }
      return null;
    }
  }
