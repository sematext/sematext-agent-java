...
public final class HttpClient3SolrRequest {
  private HttpClient3SolrRequest() {
  }

  public static void before(Trace trace, String url) {
    trace.setTag(Call.CallTag.SOLR);
    trace.setAnnotation(SolrAnnotationMaker.fromHTTPRequest(url));
  }

  public static void after(Trace trace, HttpMethodAccess method) {
    final SolrAnnotation annotation = (SolrAnnotation) trace.getAnnotation();
    if (annotation != null) {
      int code = method._$spm_tracing$_getStatusCode();
