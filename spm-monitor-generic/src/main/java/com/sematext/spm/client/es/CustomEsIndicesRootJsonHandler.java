...
public class CustomEsIndicesRootJsonHandler implements CustomJsonHandler<Map<String, Object>> {
  private static final Log LOG = LogFactory.getLog(CustomEsIndicesRootJsonHandler.class);
  ...
  public Map<String, Object> parse(InputStream inputStream, JsonFactory jsonFactory) throws IOException {
    ...
    // find "indices" child (of _all)
    CustomJsonHandlerUtil.moveToNextObject(jsonParser, true);
    CustomJsonHandlerUtil.navigateToSiblingFieldWithName(jsonParser, "indices");

    if (jsonParser.getCurrentToken() == JsonToken.END_OBJECT) {
      // no "_all/indices", we will assume just "indices" root is used
